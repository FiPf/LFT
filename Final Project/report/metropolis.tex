...
Our goal is to evaluate the expectation value of an observable $O[U]$ in the qunatizes Euclidian gauge field theory on a lattice. This is written as a path integral (functional)
\begin{equation}
...
\end{equation}
In the general case, these integrals cannot be solved analytically and depending on the application, these can be high-dimensional \cite{GattingerLang}. \\
...
\end{equation}
This estimator converges to the true value as $N \to \infty$, 
\[
I_N \xrightarrow[]{N \to \infty} \int_0^1 \exp(-x^2) \, dx.
\]
This simple example illustrates how Monte Carlo methods approximate integrals by replacing them with sample averages \cite{Monte_carlo}.
