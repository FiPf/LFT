...
\ref{eq: partition_function} and \ref{eq: partition_function}. The Metropolis algorithm is a rater inefficient way to do this and we search for 
...
For this, we introduce an artificial computer time $\tau$ and a hamiltonian dynamics to describe the developement of the field $\phi(\tau)$. In addition, 
...
By conservation of energy, we need $H = H'$, meaning $\delta H = 0$ and $P_A = 1$. However, solving differential equations numerically always interoduces errors, hence
...
$\pi$ are not used to compute the observables. They can be disregareded in each step \cite{Duane1987HMC}.\\\\
