In LFT, every calculated observable needs a statistical analysis to estimnate uts uncertainty, quantities without error
...
\begin{equation}
    \op{Var}(X) = E[(X - E[X])^2].
\end{equation}
In our case, we use as the random variable the expectation value of some observable, which we aim to calculate, $X = \braket{\mathcal{O}}$. Hence we aim to calculate
\begin{equation}
...
Unfortunately, we can only estimate the statistical error through the expression
\begin{equation}
...
In typical Monte Carlo methods in LFT, the configurations are always correlated, since we only make small steps while exploring the space of possible states. In our case, we have $\phi_{i+1} \in [\phi_i - w, \phi_i + w]$, where $w$ is the width. Elements later in the chain, for example $\phi_{i + 100}$ should me more independent from $\phi_i$. We will try to find the number of samples that can be considered independent, which is the idea behind autocorrelation. We rewrite the formula from above as follows: 
\begin{equation}
    \begin{aligned}
...
\end{equation}
Here, $\tau_{\text{int}}$ denotes the integrated autocorrelation time, which is a measure of how many steps there must be between samples to consider them as independent. This depends on the observable and needs to be recalculated for all observables. \cite{wiki:autocorrelation}
