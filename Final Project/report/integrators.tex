...
\\
In the leapfrog integration method, the field $\phi$ is 
evolved in $n$ steps of length $\varepsilon$. The conjugate momenta start with a half-step of $\frac{\varepsilon}{2}$, then $(n-1)$ full steps and finally other half-step. 
A single such step $(n = 1)$ is denoted as follows: 
\begin{align}
...
\end{equation}
This factorizes into a product of three determinants of upper diagonal matrices with $1$ on the diagpnal, hence the result equals $1$ and the integration measure is invariant. 
\\\\
...
\end{itemize}
Like leapfrog, Omelyan integration evolves the fields $\phi$ and conjugate momenta $\pi$ in a sequence of small steps. The key difference is that the momentum and position updates are distributed asymmetrically within each step according 
to a free parameter $\lambda$, which can be tuned to minimize the discretization error of the Hamiltonian. \\
A single Omelyan step of size $\varepsilon$ can be written as:
\begin{align}
    \pi &\rightarrow \pi - \lambda \frac{\varepsilon}{2} \frac{\partial S}{\partial \phi} \\
    \phi &\rightarrow \phi + \frac{\varepsilon}{2} \pi \\
    \pi &\rightarrow \pi - (1-2\lambda) \frac{\varepsilon}{2} \frac{\partial S}{\partial \phi} \\
    \phi &\rightarrow \phi + \frac{\varepsilon}{2} \pi \\
    \pi &\rightarrow \pi - \lambda \frac{\varepsilon}{2} \frac{\partial S}{\partial \phi}
\end{align}
Here, $\lambda$ is a free parameter, and the standard leapfrog scheme is recovered for $\lambda = 0$. In practice, a common 
...
\begin{itemize}
    ...
    \item Preservation of measure and reversibility: Like leapfrog, the Omelyan 2 integrator is reversible, ensuring that detailed balance in HMC is maintained.  
